...
Main.prototype = {
	loadContent: function(_) {
		Require.module("module1").then($bind(this,this.moduleLoaded),$bind(this,this.moduleFailed));
		return false;
	}
...
Require.module = function(name,loadCss) {
	if(loadCss == null) loadCss = true;
	if(Require.loaded.exists(name)) return Require.loaded.get(name);
	var p = new Promise(function(resolve,reject) {
		...
		if(loadCss) {
			...
			css.href = Require.cssPath + name + ".css";
	...
	Require.loaded.set(name,p);
	return p;
};
var common_BaseModule = $hx_exports.common.BaseModule = function() {
	var doc = window.document;
